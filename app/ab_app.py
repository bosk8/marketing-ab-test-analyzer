...
with st.sidebar:
    ...
    success_a = st.number_input(
        "Successes (conversions)", min_value=0, value=123, step=1, key="success_a")
    total_a = st.number_input("Total observations",
                              min_value=1, value=5000, step=1, key="total_a")

    st.subheader("Variant B (Treatment)")
    success_b = st.number_input(
        "Successes (conversions)", min_value=0, value=155, step=1, key="success_b")
    total_b = st.number_input("Total observations",
                              min_value=1, value=5000, step=1, key="total_b")

    st.subheader("Statistical Parameters")
    alpha = st.slider("Significance level (α)", min_value=0.01,
                      max_value=0.10, value=0.05, step=0.01)
    mde_for_power = st.slider("MDE for power calculation (pp)",
                              min_value=0.1, max_value=10.0, value=2.0, step=0.1) / 100
...
with col1:
    ...
    st.metric("Variant B", f"{pb:.4f} ({pb*100:.2f}%)",
              delta=f"{lift*100:+.3f} pp")
    st.metric("Lift", f"{lift*100:+.3f} percentage points")

# Perform statistical test
try:
    results = ztest_two_prop(
        success_a, total_a, success_b, total_b, alpha=alpha)

    with col2:
        st.subheader("Statistical Test Results")
        z_stat = results.get('z')
        p_value = results.get('p')
        lift_val = results.get('lift')

        if isinstance(z_stat, float):
            st.metric("Z-statistic", f"{z_stat:.4f}")
        if isinstance(p_value, float):
            st.metric("P-value", f"{p_value:.6f}",
                      delta="Significant" if p_value < alpha else "Not significant",
                      delta_color="normal" if p_value < alpha else "off")
        if isinstance(lift_val, float):
            st.metric("Lift", f"{lift_val*100:+.3f} pp")
    ...
    if isinstance(ci, tuple):
        ...
        st.info(
            f"**{ci[0]:.6f} to {ci[1]:.6f}** ({ci_lower_pct:+.3f} pp to {ci_upper_pct:+.3f} pp)")
    ...
    if isinstance(p_value, float) and isinstance(ci, tuple) and p_value < alpha and ci[0] > 0:
        st.success(
            "✅ **Variant B Wins** - Statistically significant improvement detected.")
    elif isinstance(p_value, float) and isinstance(ci, tuple) and ci[0] > 0 and p_value >= alpha:
        st.warning(
            "⚠️ **Inconclusive** - Positive lift but not statistically significant. Consider extending sample size.")
    elif isinstance(ci, tuple) and ci[1] < 0:
        st.info(
            "ℹ️ **Variant A Better or Equivalent** - Confidence interval suggests no improvement or decrease.")
    else:
        st.warning(
            "⚠️ **Inconclusive** - Confidence interval includes zero. Consider extending sample size.")

    # Power analysis
    st.subheader("Power Analysis")
    p_control = pa
    current_power = power(total_a, total_b, p_control,
                          min_detectable_diff=mde_for_power, alpha=alpha)
    st.metric(
    ...
    if current_power < 0.8:
        st.warning(
            f"⚠️ Power is below the recommended 80% threshold. Consider increasing sample sizes.")
    else:
        st.success(
            f"✅ Power is sufficient (≥80%) for detecting a {mde_for_power*100:.1f}pp difference.")
